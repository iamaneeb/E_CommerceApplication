...
def Register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            send_email.send()
            messages.info(request,' Thank you for registering. A verification email has been sent to your inbox. Please check your email to activate your account.')
...
def ForgetPassword(request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string("base/forget_password_verification.html",{
                'user':user,
                'domain':current_site,
                'uid':urlsafe_base64_encode(force_bytes(user.pk)),
                'token':default_token_generator.make_token(user),
            ...
            send_email=EmailMessage(mail_subject,message,to=[to_email])
            send_email.send()
            messages.info(request,"Password reset email has been send to your email address. Please check your inbox.")
            return redirect("login")
        else:
